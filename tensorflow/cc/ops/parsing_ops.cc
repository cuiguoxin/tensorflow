...
DecodeCSV::DecodeCSV(const ::tensorflow::Scope& scope, ::tensorflow::ops::Input
                     records, ::tensorflow::ops::InputList record_defaults,
...
DecodeJSONExample::DecodeJSONExample(const ::tensorflow::Scope& scope,
                                     ::tensorflow::ops::Input json_examples) {
  if (!scope.ok()) return;
...
DecodeRaw::DecodeRaw(const ::tensorflow::Scope& scope, ::tensorflow::ops::Input
...
ParseExample::ParseExample(const ::tensorflow::Scope& scope,
                           ...
                           ::tensorflow::ops::Input names,
                           ::tensorflow::ops::InputList sparse_keys,
                           ::tensorflow::ops::InputList dense_keys,
                           ::tensorflow::ops::InputList dense_defaults, const
  ...
  for (int64 i = _outputs_range["sparse_indices"].first; i < _outputs_range["sparse_indices"].second; ++i)
    this->sparse_indices.push_back(Output(ret, i));
  for (int64 i = _outputs_range["sparse_values"].first; i < _outputs_range["sparse_values"].second; ++i)
    this->sparse_values.push_back(Output(ret, i));
  for (int64 i = _outputs_range["sparse_shapes"].first; i < _outputs_range["sparse_shapes"].second; ++i)
    this->sparse_shapes.push_back(Output(ret, i));
  for (int64 i = _outputs_range["dense_values"].first; i < _outputs_range["dense_values"].second; ++i)
...
ParseSingleSequenceExample::ParseSingleSequenceExample(const
  ...
  for (int64 i = _outputs_range["context_sparse_indices"].first; i < _outputs_range["context_sparse_indices"].second; ++i)
    this->context_sparse_indices.push_back(Output(ret, i));
  for (int64 i = _outputs_range["context_sparse_values"].first; i < _outputs_range["context_sparse_values"].second; ++i)
    this->context_sparse_values.push_back(Output(ret, i));
  for (int64 i = _outputs_range["context_sparse_shapes"].first; i < _outputs_range["context_sparse_shapes"].second; ++i)
    this->context_sparse_shapes.push_back(Output(ret, i));
  for (int64 i = _outputs_range["context_dense_values"].first; i < _outputs_range["context_dense_values"].second; ++i)
    this->context_dense_values.push_back(Output(ret, i));
  for (int64 i = _outputs_range["feature_list_sparse_indices"].first; i < _outputs_range["feature_list_sparse_indices"].second; ++i)
    this->feature_list_sparse_indices.push_back(Output(ret, i));
  for (int64 i = _outputs_range["feature_list_sparse_values"].first; i < _outputs_range["feature_list_sparse_values"].second; ++i)
    this->feature_list_sparse_values.push_back(Output(ret, i));
  for (int64 i = _outputs_range["feature_list_sparse_shapes"].first; i < _outputs_range["feature_list_sparse_shapes"].second; ++i)
    this->feature_list_sparse_shapes.push_back(Output(ret, i));
  for (int64 i = _outputs_range["feature_list_dense_values"].first; i < _outputs_range["feature_list_dense_values"].second; ++i)
...
ParseTensor::ParseTensor(const ::tensorflow::Scope& scope,
                         ::tensorflow::ops::Input serialized, DataType
                         out_type) {
  if (!scope.ok()) return;
  auto _serialized = ::tensorflow::ops::AsNodeOut(scope, serialized);
  if (!scope.ok()) return;
...
StringToNumber::StringToNumber(const ::tensorflow::Scope& scope,
                               ::tensorflow::ops::Input string_tensor, const
...
StringToNumber::StringToNumber(const ::tensorflow::Scope& scope,
                               ::tensorflow::ops::Input string_tensor)
