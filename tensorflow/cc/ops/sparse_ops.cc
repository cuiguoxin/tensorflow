...
SerializeSparse::SerializeSparse(const ::tensorflow::Scope& scope,
                                 ::tensorflow::ops::Input sparse_indices,
                                 ::tensorflow::ops::Input sparse_values,
                                 ::tensorflow::ops::Input sparse_shape) {
  if (!scope.ok()) return;
  auto _sparse_indices = ::tensorflow::ops::AsNodeOut(scope, sparse_indices);
  if (!scope.ok()) return;
...
SparseAdd::SparseAdd(const ::tensorflow::Scope& scope, ::tensorflow::ops::Input
                     ...
                     ::tensorflow::ops::Input b_shape, ::tensorflow::ops::Input
                     thresh) {
  if (!scope.ok()) return;
...
SparseAddGrad::SparseAddGrad(const ::tensorflow::Scope& scope,
                             ::tensorflow::ops::Input backprop_val_grad,
                             ::tensorflow::ops::Input a_indices,
                             ::tensorflow::ops::Input b_indices,
                             ::tensorflow::ops::Input sum_indices) {
  if (!scope.ok()) return;
...
SparseConcat::SparseConcat(const ::tensorflow::Scope& scope,
                           ...
                           ::tensorflow::ops::InputList shapes, int64
                           concat_dim) {
  if (!scope.ok()) return;
...
SparseDenseCwiseAdd::SparseDenseCwiseAdd(const ::tensorflow::Scope& scope,
                                         ::tensorflow::ops::Input sp_indices,
                                         ::tensorflow::ops::Input sp_values,
                                         ::tensorflow::ops::Input sp_shape,
                                         ::tensorflow::ops::Input dense) {
  if (!scope.ok()) return;
...
SparseSplit::SparseSplit(const ::tensorflow::Scope& scope,
                         ::tensorflow::ops::Input split_dim,
                         ::tensorflow::ops::Input indices,
                         ::tensorflow::ops::Input values,
                         ::tensorflow::ops::Input shape, int64 num_split) {
  if (!scope.ok()) return;
  ...
  for (int64 i = _outputs_range["output_indices"].first; i < _outputs_range["output_indices"].second; ++i)
    this->output_indices.push_back(Output(ret, i));
  for (int64 i = _outputs_range["output_values"].first; i < _outputs_range["output_values"].second; ++i)
    this->output_values.push_back(Output(ret, i));
  for (int64 i = _outputs_range["output_shape"].first; i < _outputs_range["output_shape"].second; ++i)
...
SparseToDense::SparseToDense(const ::tensorflow::Scope& scope,
                             ::tensorflow::ops::Input sparse_indices,
                             ::tensorflow::ops::Input output_shape,
                             ::tensorflow::ops::Input sparse_values,
                             ::tensorflow::ops::Input default_value, const
