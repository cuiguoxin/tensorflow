...
ReadFile::ReadFile(const ::tensorflow::Scope& scope, ::tensorflow::ops::Input
                   filename) {
  if (!scope.ok()) return;
...
ReaderRead::ReaderRead(const ::tensorflow::Scope& scope,
...
ReaderReset::ReaderReset(const ::tensorflow::Scope& scope,
                         ::tensorflow::ops::Input reader_handle) {
  if (!scope.ok()) return;
  auto _reader_handle = ::tensorflow::ops::AsNodeOut(scope, reader_handle);
  if (!scope.ok()) return;
...
Restore::Restore(const ::tensorflow::Scope& scope, ::tensorflow::ops::Input
                 file_pattern, ::tensorflow::ops::Input tensor_name, DataType
                 dt, const Restore::Attrs& attrs) {
  if (!scope.ok()) return;
...
RestoreSlice::RestoreSlice(const ::tensorflow::Scope& scope,
                           ::tensorflow::ops::Input file_pattern,
                           ::tensorflow::ops::Input tensor_name,
                           ::tensorflow::ops::Input shape_and_slice, DataType
                           dt, const RestoreSlice::Attrs& attrs) {
  if (!scope.ok()) return;
  auto _file_pattern = ::tensorflow::ops::AsNodeOut(scope, file_pattern);
  if (!scope.ok()) return;
  auto _tensor_name = ::tensorflow::ops::AsNodeOut(scope, tensor_name);
...
RestoreV2::RestoreV2(const ::tensorflow::Scope& scope, ::tensorflow::ops::Input
                     prefix, ::tensorflow::ops::Input tensor_names,
                     ::tensorflow::ops::Input shape_and_slices, const
...
Save::Save(const ::tensorflow::Scope& scope, ::tensorflow::ops::Input filename,
           ::tensorflow::ops::Input tensor_names, ::tensorflow::ops::InputList
           data) {
  if (!scope.ok()) return;
  auto _filename = ::tensorflow::ops::AsNodeOut(scope, filename);
  if (!scope.ok()) return;
...
SaveSlices::SaveSlices(const ::tensorflow::Scope& scope,
                       ::tensorflow::ops::Input filename,
                       ::tensorflow::ops::Input tensor_names,
                       ::tensorflow::ops::Input shapes_and_slices,
...
SaveV2::SaveV2(const ::tensorflow::Scope& scope, ::tensorflow::ops::Input
               prefix, ::tensorflow::ops::Input tensor_names,
               ::tensorflow::ops::Input shape_and_slices,
               ::tensorflow::ops::InputList tensors) {
  if (!scope.ok()) return;
  auto _prefix = ::tensorflow::ops::AsNodeOut(scope, prefix);
  if (!scope.ok()) return;
  auto _tensor_names = ::tensorflow::ops::AsNodeOut(scope, tensor_names);
...
ShardedFilename::ShardedFilename(const ::tensorflow::Scope& scope,
                                 ::tensorflow::ops::Input basename,
                                 ::tensorflow::ops::Input shard,
